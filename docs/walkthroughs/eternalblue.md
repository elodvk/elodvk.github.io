---
title: EternalBlue (MS17-010) — Technical Overview & Security Mitigation
date: 2025-01-30
difficulty: N/A
os: N/A
authors:
...
- SMB Exploitation
- Security Mitigation
description: Technical overview of the EternalBlue exploit (CVE-2017-0144). Explains
  the underlying SMBv1 protocol vulnerability, real-world impacts like WannaCry, and
  key security mitigation strategies.
image: assets/eternalblue/eternalblue_banner.png
---


![EternalBlue (MS17-010) Banner](assets/eternalblue/eternalblue_banner.png)

# EternalBlue (MS17-010) — Technical Exploit Overview & Hardening
...
| Property             | Value                                  |
| -------------------- | -------------------------------------- |
| **Vulnerability ID**  | [CVE-2017-0144](https://nvd.nist.gov/vuln/detail/CVE-2017-0144) |
| **Common Name**      | EternalBlue / MS17-010                 |
| **Severity**         | Critical (CVSS 10.0)                   |
| **Protocol**         | Server Message Block Version 1 (SMBv1) |
| **Impact**           | Remote Code Execution (RCE) / SYSTEM    |
| **Remediation**      | Microsoft Security Update MS17-010     |

---

## Exploit Execution Flow

The following diagram illustrates the network and memory exploitation flow of EternalBlue:

```mermaid
sequenceDiagram
    participant Attacker
    participant Target as Target Windows OS (SMBv1)

    Attacker->>Target: 1. Connection request via TCP 445
    Attacker->>Target: 2. Send malformed Trans2 request packet
    Note over Target: 3. Integer overflow in Srv!SrvSmbDeliverTransaction
    Note over Target: 4. Out-of-bounds write / heap corruption
    Note over Target: 5. Execute kernel shellcode
    Target-->>Attacker: 6. Returns SYSTEM reverse shell / payload execution
```
...
!!! note "Microsoft Security Patches"
    Microsoft released security patches for these systems in March 2017 (MS17-010) for supported versions and made patches available for unsupported systems (such as Windows XP and Windows Server 2003) as part of an emergency update.
