...
## HTB Rebound — Full Walkthrough & Writeup

**Rebound** is an insane-difficulty Windows Active Directory machine from Hack The Box. This writeup details the complete attack path, starting from anonymous SMB access and RID brute-forcing to identify domain users, followed by Kerberoasting without pre-authentication to compromise the `ldap_monitor` service account. We then perform password spraying to gain access to `oorend`, execute a Shadow Credentials attack on `winrm_svc`, relay local authentication via `RemotePotato0`, and leverage Constrained Delegation alongside Resource-Based Constrained Delegation (RBCD) to dump NTDS.dit and compromise the domain controller.
...
| **Starting Credentials** | `guest` / Null SMB Session             |
...
!!! warning "GMSA Security Risks"
    It is not a good idea to grant a user account read password permissions on a gMSA password.
...
### 1. Disable Kerberoasting Without Pre-Authentication
By leaving Pre-Authentication disabled on `jjones`, we could perform Kerberoasting directly against the domain controller without credentials, leading to the compromise of `ldap_monitor`.
- **Remediation**: Enforce Kerberos Pre-Authentication on all Active Directory user accounts.

### 2. Remediate Password Reuse
The service account `ldap_monitor` shared its password with the standard domain user `oorend`.
- **Remediation**: Establish strict password policies ensuring that all domain accounts have unique, complex passwords, particularly service accounts with AD administrative delegations.

### 3. Mitigate Shadow Credentials and RBCD
We leveraged direct write permissions on the `SERVICE USERS` OU to perform a Shadow Credentials attack on `winrm_svc` and modify RBCD attributes on computer accounts.
- **Remediation**: Audit OU delegations and computer object permissions regularly. Limit write permissions over computer objects and administrative attributes (such as `msDS-KeyCredentialLink` and `msDS-AllowedToActOnBehalfOfOtherIdentity`) to Domain/Enterprise Admins.

### 4. RemotePotato0 Mitigation (Local Relay Hardening)
The local relay attack using RemotePotato0 relies on NTLM authentication over local RPC interfaces.
- **Remediation**: Disable NTLM authentication where possible, or enable NTLM Auditing and restrict local NTLM relaying. Implement RPC endpoint hardening and apply Microsoft security patches that mitigate potato-based privilege escalations.
