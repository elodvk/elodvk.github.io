...
```shell
Bilash@htb[/htb]$ nmap -p 80,443,8000,8080,8180,8888,10000 --open -oA web_discovery -iL scope_list
```
...
!!! Note
    Not all hosts in the scope list above will be accessible when spawning the target below. There will be separate, similar, exercises at the end of this section in order to reproduce much of what is shown here.

```shell
Bilash@htb[/htb]$ sudo  nmap -p 80,443,8000,8080,8180,8888,10000 --open -oA web_discovery -iL scope_list 
...
```shell
Bilash@htb[/htb]$ sudo nmap --open -sV 10.129.201.50
...
```shell
Bilash@htb[/htb]$ sudo apt install eyewitness
```
...
```shell
Bilash@htb[/htb]$ eyewitness --web -x web_discovery.xml -d inlanefreight_eyewitness
...
```shell
Bilash@htb[/htb]$ wget https://github.com/michenriksen/aquatone/releases/download/v1.7.0/aquatone_linux_amd64_1.7.0.zip
```

```shell
Bilash@htb[/htb]$ unzip aquatone_linux_amd64_1.7.0.zip 
...
```shell
Bilash@htb[/htb]$ cat web_discovery.xml | ./aquatone -nmap
